...
/*: "http://app. :*/
fileprivate let kPlatformData:String = "privacy server safe your zonehttp:"

/*: .com" :*/
fileprivate let app_arrayKey:String = ".comenvironment link mask report"
...
class ActivePromptRequestModel: NSObject {
    ...
    override init() {
        ...
        self.requestServer = (String(kPlatformData.suffix(5)) + "//app.") + "\(k_statusLogShouldTitle)" + (String(app_arrayKey.prefix(4)))
    }
}

/// 通用Model
//: struct AppBaseResponse: HandyJSON {
struct ConditionBaseResponse: HandyJSON {
    //: var errno: Int!
...
public struct FirErrorResponse {
    //: let errorCode: Int
    let errorCode: Int
    //: let errorMsg: String
    let errorMsg: String
