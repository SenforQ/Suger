...
fileprivate let constReduceData:[Character] = ["m","f","/","r","e"]
fileprivate let userActionPath:[Character] = ["c","h","a","r","g","e","/","c","r","e","a","t","e","A","p","p","l","e","P","a","y"]

/*: "productId" :*/
fileprivate let kArrayName:String = "pmaked"
fileprivate let showCurrencyId:String = "again will not layer presentuctId"

/*: "source" :*/
fileprivate let main_screenContent:String = "sonetworkrce"

/*: "orderNum" :*/
fileprivate let k_warnMsg:[Character] = ["o","r","d"]
fileprivate let notiCreateData:String = "erNummic empty total"

/*: "mf/recharge/applePayNotify" :*/
fileprivate let constObserverId:String = "mf/recindicator ok area or"
fileprivate let show_contentValue:String = "view copy behavior scale oke/app"
fileprivate let notiBodyName:String = "content centerlePayN"

/*: "reportMoney" :*/
fileprivate let k_stopOnData:String = "rnoticepo"

/*: "transactionId" :*/
fileprivate let show_zoneText:String = "taccountan"
fileprivate let k_removeValue:String = "break value contact access labonId"

/*: "mf/AutoSub/AppleCreateOrder" :*/
fileprivate let kVerticalFormat:[Character] = ["m","f","/","A","u","t","o","S","u","b","/","A","p","p","l","e","C","r","e","a","t","e","O","r","d","e","r"]

/*: "orderId" :*/
fileprivate let dataVersionName:[UInt8] = [0x84,0x99,0x8f,0x8e,0x99,0xa2,0x8f]

/*: "mf/AutoSub/ApplePaySuccess" :*/
fileprivate let dataInfoValue:String = "mf/Autthat self region link"
fileprivate let data_rootId:[Character] = ["l","e","P","a"]
fileprivate let kNativeId:[Character] = ["y","S","u","c","c","e","s","s"]

/*: "App" :*/
fileprivate let constAccessRegionStr:String = "head prompt item media nativeApp"

/*: "OrderTransactionInfo_Cache" :*/
fileprivate let k_mainName:[Character] = ["O","r","d","e","r","T","r","a","n","s","a","c","t","i","o","n","I","n","f","o","_","C","a","c","h"]
fileprivate let main_maxUrl:[Character] = ["e"]

/*: "OrderTransactionInfo_Subscribe_Cache" :*/
fileprivate let data_presentationValue:[UInt8] = [0xc3,0xfe,0xe8,0xe9,0xfe,0xd8,0xfe,0xed,0xe2,0xff,0xed,0xef,0xf8,0xe5,0xe3,0xe2,0xc5,0xe2,0xea,0xe3,0xd3,0xdf,0xf9,0xee,0xff,0xef,0xfe,0xe5,0xee,0xe9,0xd3,0xcf,0xed,0xef,0xe4,0xe9]

/*: "verifyData" :*/
fileprivate let noti_originalId:[UInt8] = [0x39,0x2a,0x3d,0x26,0x29,0x36,0xb,0x2e,0x3b,0x2e]

private func frameField(record num: UInt8) -> UInt8 {
    return num ^ 79
}

/*: " 未知的交易类型" :*/
fileprivate let data_observerPath:String = " \u{672a}知的交"
fileprivate let app_sceneMessage:[Character] = ["易","\u{7c7b}","型"]
...
enum ActuallyTitleConvertible: String {
    //: case unknow            = "未知类型"
    case unknow = "未知类型"
...
typealias IAPcompletionHandle = (ActuallyTitleConvertible, Double, InciteType) -> Void

//: class AppleIAPManager: NSObject {
class ResignManager: NSObject {
    ...
    private var currentPayType: InciteType = .Pay // 当前支付类型
    ...
    override private init() {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(range),
...
private extension ResignManager {
    /// 【购买】创建业务订单
    /// - Parameters:
    ///   - productId: 产品Id
    ///   - block: 回调
    //: fileprivate func req_pay_createAppleOrder(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
    func anySource(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = ActivePromptRequestModel()
        //: reqModel.requestPath = "mf/recharge/createApplePay"
        reqModel.requestPath = (String(constReduceData) + String(userActionPath))
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["productId"] = productId
        dict[(kArrayName.replacingOccurrences(of: "make", with: "ro") + String(showCurrencyId.suffix(5)))] = productId
        //: dict["source"] = source
        dict[(main_screenContent.replacingOccurrences(of: "network", with: "u"))] = source
        //: reqModel.params = dict
        reqModel.params = dict
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        WindowRequestTool.post(model: reqModel) { succeed, result, _ in
            ...
            if let value = dict?[(String(k_warnMsg) + String(notiCreateData.prefix(5)))] as? String {
                //: orderId = value
                orderId = value
            }
    ...
    func currency(_ transactionId: String, params: [String: String]) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = ActivePromptRequestModel()
        //: reqModel.requestPath = "mf/recharge/applePayNotify"
        reqModel.requestPath = (String(constObserverId.prefix(6)) + "harg" + String(show_contentValue.suffix(5)) + String(notiBodyName.suffix(6)) + "otify")
        //: reqModel.params = params
        reqModel.params = params
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        WindowRequestTool.post(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 2) {
                    ...
                    self.artifact(transactionId, .Pay)
                }
                //: return
                return
            }
            ...
            let reportMoney: Double = {
                ...
                if let d = dict?[(k_stopOnData.replacingOccurrences(of: "notice", with: "e") + "rtMoney")] as? Double { return d }
                //: return 0
                return 0
                //: }()
            }()
            ...
            let newPayCacheList = self.payCacheList.filter { $0[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))] != transactionId }
            //: let diskPath = self.getPayCachePath()
            let diskPath = self.microscopicPath()
...
private extension ResignManager {
    ...
    func at(productId: String, source: Int, handle: @escaping (String?, Bool) -> Void) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = ActivePromptRequestModel()
        //: reqModel.requestPath = "mf/AutoSub/AppleCreateOrder"
        reqModel.requestPath = (String(kVerticalFormat))
        //: var dict = Dictionary<String, Any>()
        var dict = [String: Any]()
        //: dict["productId"] = productId
        dict[(kArrayName.replacingOccurrences(of: "make", with: "ro") + String(showCurrencyId.suffix(5)))] = productId
        ...
        WindowRequestTool.post(model: reqModel) { succeed, result, _ in
            ...
            if let value = dict?[String(bytes: dataVersionName.map{$0^235}, encoding: .utf8)!] as? String {
                //: orderId = value
                orderId = value
            }
            //: handle(orderId, succeed)
    ...
    func search(_ transactionId: String, params: [String: String]) {
        //: let reqModel = AppRequestModel.init()
        let reqModel = ActivePromptRequestModel()
        //: reqModel.requestPath = "mf/AutoSub/ApplePaySuccess"
        reqModel.requestPath = (String(dataInfoValue.prefix(6)) + "oSub/App" + String(data_rootId) + String(kNativeId))
        //: reqModel.params = params
        reqModel.params = params
        //: AppRequestTool.startPostRequest(model: reqModel) { succeed, result, errorModel in
        WindowRequestTool.post(model: reqModel) { succeed, result, errorModel in
            ...
            guard succeed == true || errorModel?.errorCode == 405 else { // 验证接口失败，重试接口
                ...
                DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + 3) {
                    ...
                    self.artifact(transactionId, .Subscribe)
                }
                //: return
                return
            }
            ...
            let newSubscribeCacheList = self.subscribeCacheList.filter { $0[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))] != transactionId }
            //: let diskPath = self.getSubscribeCachePath()
            let diskPath = self.local()
...
extension ResignManager {
    ...
    private func all(payType: InciteType) -> [[String: String]] {
    ...
    private func microscopicPath() -> String {
        //: let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        let documentDirectoryPath = NSSearchPathForDirectoriesInDomains(.documentDirectory, .userDomainMask, true).first ?? ""
        //: let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent("App")
        let appDirectoryPath = (documentDirectoryPath as NSString).appendingPathComponent((String(constAccessRegionStr.suffix(3))))
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent((String(k_mainName) + String(main_maxUrl)))
        //: return filePath
        return filePath
    }
    ...
    private func local() -> String {
        ...
        let filePath = (appDirectoryPath as NSString).appendingPathComponent(String(bytes: data_presentationValue.map{$0^140}, encoding: .utf8)!)
        //: return filePath
        return filePath
    }
    ...
    fileprivate func overHireData(_ transactionId: String, _ payType: InciteType) -> String? {
        // 有未完成的订单，先取缓存
        ...
        case .Pay:
            ...
            paramsArr = self.payCacheList.filter { $0[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))] == transactionId }
        //: case .Subscribe:
        case .Subscribe:
            //: paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            paramsArr = self.subscribeCacheList.filter { $0[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))] == transactionId }
        }
        //: if paramsArr.count > 0 && paramsArr.first!["verifyData"] != nil {
        if paramsArr.count > 0, paramsArr.first![String(bytes: noti_originalId.map{frameField(record: $0)}, encoding: .utf8)!] != nil {
            //: return paramsArr.first!["verifyData"]
            return paramsArr.first![String(bytes: noti_originalId.map{frameField(record: $0)}, encoding: .utf8)!]
        }
...
extension ResignManager {
    ...
    func dismissRecordQuery() {
        ...
        for dict in self.payCacheList {
            ...
            exceptUserRetry(dict[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))], .Pay)
        }
        ...
        for dict in self.subscribeCacheList {
            ...
            exceptUserRetry(dict[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))], .Subscribe)
        }
    }
    ...
    private func exceptUserRetry(_ transactionId: String?, _ payType: InciteType) {
        ...
        artifact(transactionId, payType)
    }
}
...
extension ResignManager {
    ...
    func dataConverterHandle(productId: String, payType: InciteType, source: Int = 0, handle: @escaping IAPcompletionHandle) {
        //: iap_initData()
        ratingMethod()
        ...
        case .Pay:
            ...
            anySource(productId: productId, source: source) { [weak self] orderId, succeed in
                ...
                self.stockGenRequest(productId)
            }

        //: case .Subscribe:
        case .Subscribe:
            //: req_subscribe_createAppleOrder(productId: productId, source: source) { [weak self] orderId, succeed in
            at(productId: productId, source: source) { [weak self] orderId, succeed in
                //: guard let self = self else { return }
                guard let self = self else { return }
                //: guard succeed == true && orderId != nil else {
                guard succeed == true && orderId != nil else { // 订单创建失败
    ...
    fileprivate func stockGenRequest(_ productId: String) {
...
extension ResignManager: SKProductsRequestDelegate {
    // 查询apple内购商品成功回调
...
extension ResignManager: SKPaymentTransactionObserver {
    /// 2.2 apple内购完成回调
        ...
        for transaction in transactions {
            ...
            case .purchased: // 交易成功
                ...
                if transaction.original != nil && createOrderId == nil { // 启动自动续订时，不需要调用服务端验证接口
                    ...
                    artifact(transaction.transactionIdentifier!, self.currentPayType)
                }
                // 移除苹果支付系统缓存
            ...
            @unknown default:
                ...
                createOrderId = nil
                //: fatalError(" 未知的交易类型")
                fatalError((data_observerPath + String(app_sceneMessage)))
            }
        }
    }
    ...
    fileprivate func artifact(_ transactionId: String, _ payType: InciteType) {
        //: guard let receiptStr = getVerifyData(transactionId, payType) else {
        guard let receiptStr = overHireData(transactionId, payType) else {
        ...
        if createOrderId != nil { // 正常支付流程
            ...
            case .Pay:
                ...
                if self.payCacheList.filter({ $0[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))] == transactionId || $0[String(bytes: dataVersionName.map{$0^235}, encoding: .utf8)!] == createOrderId }).count == 0 { // 防止重复添加缓存数据
                    //: let cacheDict = ["transactionId": transactionId,
                    let cacheDict = [(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4))): transactionId,
                                     //: "orderId": createOrderId!,
                                     String(bytes: dataVersionName.map{$0^235}, encoding: .utf8)!: createOrderId!,
                                     //: "verifyData": receiptStr]
                                     String(bytes: noti_originalId.map{frameField(record: $0)}, encoding: .utf8)!: receiptStr]
            ...
            case .Subscribe:
                ...
                if self.subscribeCacheList.filter({ $0[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))] == transactionId || $0[String(bytes: dataVersionName.map{$0^235}, encoding: .utf8)!] == createOrderId }).count == 0 { // 防止重复添加缓存数据
                    //: let cacheDict = ["transactionId": transactionId,
                    let cacheDict = [(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4))): transactionId,
                                     //: "orderId": createOrderId!,
                                     String(bytes: dataVersionName.map{$0^235}, encoding: .utf8)!: createOrderId!,
        ...
        case .Pay:
            ...
            let paramsArr = self.payCacheList.filter { $0[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))] == transactionId }
            //: guard paramsArr.count > 0 else { return }
            guard paramsArr.count > 0 else { return }
            //: req_pay_uploadAppletransaction(transactionId, params: paramsArr.first!)
            currency(transactionId, params: paramsArr.first!)

        //: case .Subscribe:
        case .Subscribe:
            //: let paramsArr = self.subscribeCacheList.filter({$0["transactionId"] == transactionId})
            let paramsArr = self.subscribeCacheList.filter { $0[(show_zoneText.replacingOccurrences(of: "account", with: "r") + "sacti" + String(k_removeValue.suffix(4)))] == transactionId }
            //: guard paramsArr.count > 0 else { return }
            guard paramsArr.count > 0 else { return }
            //: req_subscribe_uploadAppletransaction(transactionId, params: paramsArr.first!)
            search(transactionId, params: paramsArr.first!)
        }
    }
}
