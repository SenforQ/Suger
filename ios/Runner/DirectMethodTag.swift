...
class DirectMethodTag {
    //: static let shared = AppPermissionTool()
    static let shared = DirectMethodTag()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func key(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func atScript(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
    ...
    func launch(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
