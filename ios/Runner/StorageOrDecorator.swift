...
/*: "USD" :*/
fileprivate let noti_networkMsg:String = "componentD"
...
class StorageOrDecorator: NSObject {
    //: static let shared = AppAdjustManager()
    static let shared = StorageOrDecorator()
    ...
    func run() {
        ...
        let adjustConfig = ADJConfig(appToken: main_duePackageKey, environment: environment)
        ...
        StorageOrDecorator.add(token: app_matchFormat)
    }
}
...
extension StorageOrDecorator: AdjustDelegate {
    /// 获取设备id
    //: class func getAdjustAdid() -> String {
    class func beginColor() -> String {
    ...
    class func media(token: String, count: Double) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: event?.setRevenue(count, currency: "USD")
        event?.setRevenue(count, currency: (noti_networkMsg.replacingOccurrences(of: "component", with: "US")))
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
    }
    ...
    class func revenue(token: String) {
        //: let event = ADJEvent(eventToken: token)
        let event = ADJEvent(eventToken: token)
        //: Adjust.trackEvent(event)
        Adjust.trackEvent(event)
